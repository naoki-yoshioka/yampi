...
# ifndef BOOST_NO_CXX11_UNIFIED_INITIALIZATION_SYNTAX
    error(int const error_code, std::string const& where)
     : std::runtime_error{generate_what_string(error_code, where).c_str()}, error_class_{generate_error_class(error_code)}
    { }
# else
    error(int const error_code, std::string const& where)
     : std::runtime_error(generate_what_string(error_code, where).c_str()), error_class_(generate_error_class(error_code))
...
# endif
   ...
   private:
    int generate_error_class(int const error_code)
    {
