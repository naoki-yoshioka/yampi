...
# endif
  ...
  {
    assert(last >= first);
    ::yampi::blocking_receive(first, last-first, source, tag, communicator);
  }
  ...
  { ::yampi::blocking_receive(boost::begin(values), boost::end(values), source, tag, communicator); }
  ...
  blocking_receive(Value& value, ::yampi::rank const source, ::yampi::tag const tag, ::yampi::communicator const communicator, ::yampi::ignore_status_t const)
  {
# ifndef BOOST_NO_CXX11_AUTO_DECLARATIONS
    auto const error_code
...
# endif
  ...
  blocking_receive(::yampi::rank const source, ::yampi::tag const tag, ::yampi::communicator const communicator, ::yampi::ignore_status_t const)
  {
# ifndef BOOST_NO_CXX11_AUTO_DECLARATIONS
#   ifndef BOOST_NO_CXX11_UNIFIED_INITIALIZATION_SYNTAX
    auto result = Value{};
...
# endif
  ...
  typename YAMPI_enable_if<
    ::yampi::is_contiguous_iterator<ContiguousIterator>::value
      and ::yampi::has_corresponding_mpi_data_type<typename std::iterator_traits<ContiguousIterator>::value_type>::value,
    void>::type
  blocking_receive(ContiguousIterator const first, int const length, ::yampi::rank const source, ::yampi::tag const tag, ::yampi::communicator const communicator, ::yampi::ignore_status_t const)
  {
    typedef typename std::iterator_traits<ContiguousIterator>::value_type value_type;
...
# endif
  ...
  typename YAMPI_enable_if<
    ::yampi::is_contiguous_iterator<ContiguousIterator>::value
      and ::yampi::has_corresponding_mpi_data_type<typename std::iterator_traits<ContiguousIterator>::value_type>::value,
    void>::type
  blocking_receive(ContiguousIterator const first, ContiguousIterator const last, ::yampi::rank const source, ::yampi::tag const tag, ::yampi::communicator const communicator, ::yampi::ignore_status_t const ignore)
  {
    assert(last >= first);
    ::yampi::blocking_receive(first, last-first, source, tag, communicator, ignore);
  }
  ...
  typename YAMPI_enable_if<
    ...
    void>::type
  blocking_receive(ContiguousRange& values, ::yampi::rank const source, ::yampi::tag const tag, ::yampi::communicator const communicator, ::yampi::ignore_status_t const ignore)
  { ::yampi::blocking_receive(boost::begin(values), boost::end(values), source, tag, communicator, ignore); }
}
