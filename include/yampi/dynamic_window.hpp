...
namespace yampi
{
    ...
    : public ::yampi::window_base< ::yampi::dynamic_window >
  {
   ...
   public:
    bool operator==(dynamic_window const& other) const noexcept { return mpi_win_ == other.mpi_win_; }
    ...
    void swap(dynamic_window& other) noexcept(YAMPI_is_nothrow_swappable<MPI_Win>::value)
    {
      using std::swap;
  ...
  inline bool operator!=(::yampi::dynamic_window const& lhs, ::yampi::dynamic_window const& rhs) noexcept(noexcept(lhs == rhs))
  { return not (lhs == rhs); }

  inline void swap(::yampi::dynamic_window& lhs, ::yampi::dynamic_window& rhs) noexcept(noexcept(lhs.swap(rhs)))
  { lhs.swap(rhs); }
}
# endif // MPI_VERSION >= 3
